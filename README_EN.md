...
**Geo Aisaray** is a geographic engine for zones, routes, and movement monitoring. It is designed for dispatchers, logistics teams, and owners of location-based operations and provides one view of geofences, routes, and movement events.

- Public URL: https://geo.aisaray.ru/
- Core entities: geofences, points, routes, tracked objects, events, and geographic reference data.
- Key capabilities: geographic interface, zone management, routes, object monitoring, entry and exit events, geographic administration, applied geo interface.

## Why the Product Is Needed

A real workflow rarely consists of a single action. Users must understand the initial situation, gather information, choose the next step, coordinate a decision, and return to the result later. Geo Aisaray provides one domain-specific environment for this work. It preserves context between stages and shows not only an individual item, but also its place in the overall process.

The product’s main value is not the number of screens, but workflow continuity. Geofences, points, routes, tracked objects, events, and geographic reference data are treated as parts of one user journey. New participants understand the current state faster, experienced specialists spend less time matching information manually, and managers gain a clear overview without requiring every step to be retold.
...
The roles Dispatcher, Geospatial analyst, and Reference-data administrator work with the same domain model from different perspectives. Consistent names for objects and states reduce ambiguity during handoffs. A responsible participant can reference a specific object and expected result instead of repeating the entire context.

## Main Sections

The public description covers these user capabilities:

- **Geographic interface** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Zone management** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Routes** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Object monitoring** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Entry and exit events** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Geographic administration** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Applied geo interface** — a distinct part of the workflow, connected to the product model and a clear user outcome.
...
### Geo home
![Geo home](docs/screenshots/geo_home.png)
